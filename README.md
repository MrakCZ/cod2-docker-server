...
CoD2x je načtena přes `LD_PRELOAD` - před spuštěním `cod2_lnxded` se načte `libCoD2x.so` která hookuje herní funkce a přidává nové možnosti (vyšší tickrate, HWID bany, opravy bugů). Hráči s CoD2x klientem získají plnou funkcionalitu, vanilla klienti (1.3) se mohou připojit také.
